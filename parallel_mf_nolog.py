...
def main(input_args=None):
    ...
    clipped_baseoutfile = args.output + '_scaleclip'
    clipped_baseoutfilehdr = baseoutfile+'_scaleclip.hdr'


    print('Started processing input file: "%s"'%str(radiance_file))
    img = envi.open(radiance_filehdr,image=radiance_file)
    img_mm = img.open_memmap(interleave='source',writeable=False)
    print('Memmap openned: "%s"'%str(radiance_file))
    nrows,nbands,ncols = img_mm.shape

    # define active channels wrt target gas + measurement units
    if 'wavelength' not in img.metadata.keys():
        print('wavelength field not found in input header')
        sys.exit(0)
    ...
    if 'ch4' in args.library:
        ...
        print(f'CH4 active chanels: {active}')
    elif 'co2' in args.library:
        active = [np.argmin(np.abs(wavelengths - x)) for x in CO2_WL]
        print(f'CO2 active chanels: {active}')
    else:
        print('could not set active range - neither co2 nor ch4 found in library name')
        sys.exit(0)
    ...
    rayargs = {'_temp_dir': args.ray_temp_dir, 'ignore_reinit_error': True}
    if args.num_cores != -1:
...
def looshrinkage(I_zm,alphas,nll,n,I_reg=[]):
    ...
    for i,alpha in enumerate(alphas):
        try:
            ...
            printing('looshrinkage encountered a LinAlgError')
...
def mf_one_column(col, img_mm, bgminsamp, outimg_mm_shape, bgimg_mm_shape, abscf, args):
    ...
    if bgmodes > 1:
        ...
        if (bgulab<0).all():
            printing('all clusters rejected, proceeding without rejection (beware!)')
