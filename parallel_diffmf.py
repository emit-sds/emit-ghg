...
def main(input_args=None):
    ...
    if args.fg_output_file is not None:
        outmeta['data type'] = np2envitype(np.uint8)
        outmeta['data ignore value'] = 0
        outmeta['bands'] = 1
        output_ds = envi.create_image(envi_header(args.fg_output_file),outmeta,force=True,ext='')
        del output_ds
        write_bil_chunk(np.ones(output_shape)*args.nodata_value, args.fg_output_file, 0, output_shape)
    ...
    for _ce, ce in enumerate(chunk_edges[:-1]):
        ...
        logging.debug("adding foreground mask")            
        if args.fg_input_file is not None:
            fg_input_mask = envi.open(envi_header(args.fg_input_file)).open_memmap(interleave='bip')[ce:chunk_edges[_ce+1],:,0] != 0
            good_pixel_mask = np.where(fg_input_mask, False, good_pixel_mask)
        ...
        if args.fg_output_file is not None:
            output_fg_mask = diffmf_nsigma_mask(output_retr_dat,args)
            write_bil_chunk(output_fg_mask, args.fg_output_file, ce, chunk_shape)
